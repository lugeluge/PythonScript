...
formats = {'mso': ("doc", "xls", "ppt", "png"), 'oo': ("odt", "ods", "ppt", "png"), 'pdf': ("pdf", "pdf", "pdf", "pdf"),
           'txt': ("txt", "csv", "txt", "png"), 'html': ("zip", "html", "swf", "png"), }
format = "mso"
...
parser.add_option("-m", "--modified-only", dest="modified_only", action="store_true",
...
parser.add_option("--format", dest="format",
                  help="Sets the format that documents are downloaded as.  Please see readme.txt for valid formats.")
parser.add_option("--document-format", dest="docformat",
                  help="Sets the format that word processor documents are downloaded as.  Please see readme.txt for valid formats.")
parser.add_option("--spreadsheet-format", dest="spdformat",
                  help="Sets the format that spreadsheets are downloaded as.  Please see readme.txt for valid formats.")
parser.add_option("--presentation-format", dest="prsformat",
                  help="Sets the format that presentations are downloaded as.  Please see readme.txt for valid formats.")
parser.add_option("--drawing-format", dest="drwformat",
                  help="Sets the format that drawings are downloaded as.  Please see readme.txt for valid formats.")
...
parser.add_option("-d", "--datetag", dest="datetag", action="store_true",
...
cfg = {'email': '', 'password': '', 'path': '', 'override': False, 'format': 'mso',
       'feed': 'https://docs.google.com/feeds/default/private/full', 'document_format': '', 'spreadsheet_format': '',
       'presentation_format': '', 'drawing_format': '', 'use_unicode': False, 'use_datetag': False,
       'use_folders': False, 'modified_only': False, 'clean_up': False, 'illegal_characters': '<>:"/\|?*^'}
...
def login():
    ...
    values = {'Email': cfg['email'], 'Passwd': cfg['password'], 'accountType': 'HOSTED_OR_GOOGLE', 'service': 'writely',
              'source': user_agent, 'GData-Version': '3.0'}
    values2 = {'Email': cfg['email'], 'Passwd': cfg['password'], 'accountType': 'HOSTED_OR_GOOGLE', 'service': 'wise',
               'source': user_agent, 'GData-Version': '3.0'}
    ...
    try:
        ...
        if debug:
            ...
            debugmsg({'Email Address': cfg['email'], 'Password': sensitive(cfg['password'], '*'),
                      'Post Data 1': urllib.urlencode(values), 'Post Data 2': urllib.urlencode(values2),
                      'Post URL': url})
...
def getFeed(auth, feed, tries=0):
    ...
    try:
        ...
        if debug:
            debugmsg({'Feed URL': url, 'Authorization': 'GoogleLogin auth=' + sensitive(auth[1], '*'),
                      'Modified Only': str(cfg['modified_only'])})
    except:
        if tries < 3:
...
def parseFeed(xml, auth=('', '', '')):
    ...
    for node in dom.getElementsByTagName('entry'):
        ...
        if doc.find("spreadsheet") != -1 and cfg['spreadsheet_format'] in ['csv', 'tsv', 'html']:
            ...
            if sheets == []:
                ...
                j = 0
                for k in sheets:
                    docs.append({'doc': doc, 'title': title + " (" + k + ")", 'parent': parent, 'id': id,
                                 'src': src + "&gid=" + str(j), 'etag': etag})
                    j += 1
        else:
            if src != '':
                docs.append({'doc': doc, 'title': title, 'parent': parent, 'id': id, 'src': src, 'etag': etag})
    ...
    for x in docs:
        ...
        title = x['title']
        if type == "document":
            urls.append({'parent': x['parent'], 'id': x['id'], 'type': type, 'title': title, 'etag': x['etag'],
                         'url': x['src'] + "&exportFormat=" + cfg['document_format']})
        elif type == "spreadsheet":
            urls.append({'parent': x['parent'], 'id': x['id'], 'type': type, 'title': title, 'etag': x['etag'],
                         'url': x['src'] + "&exportFormat=" + cfg['spreadsheet_format']})
        elif type == "pdf":
            urls.append({'parent': x['parent'], 'id': x['id'], 'type': type, 'title': title, 'etag': x['etag'],
                         'url': x['src']})
        elif type == "presentation":
            urls.append({'parent': x['parent'], 'id': x['id'], 'type': type, 'title': title, 'etag': x['etag'],
                         'url': x['src'] + "&exportFormat=" + cfg['presentation_format']})
        elif type == "drawing":
            urls.append({'parent': x['parent'], 'id': x['id'], 'type': type, 'title': title, 'etag': x['etag'],
                         'url': x['src'] + "&exportFormat=" + cfg['drawing_format']})
        elif type == "folder":
...
def getFile(doc, headers, dname, tries=0):
    ...
    if cfg['modified_only'] and doc['id'] in etags and doc['etag'] == etags[doc['id']] and doc[
        'id'] in prev_successfuldocs.keys() and does_file_exist:
    ...
    try:
        ...
        if (f.info().getheader('Content-Disposition') != None and f.info().getheader(
                'Content-Disposition') != "" and not cfg['use_unicode']):
...
def download(auth, urls):
    ...
    for i in docs:
        if i['type'] != "spreadsheet":
            result = getFile(i, {'Authorization': 'GoogleLogin auth=' + auth[1], 'GData-Version': '3.0'},
                             cfg['path'] + auth[0] + dtpath)
            noerror = result['success'] and noerror
            files.append(result['file'])
        else:
            result = getFile(i, {'Authorization': 'GoogleLogin auth=' + auth[2], 'GData-Version': '3.0'},
                             cfg['path'] + auth[0] + dtpath)
            noerror = result['success'] and noerror
            files.append(result['file'])
...
if auth[0] != "" and auth[1] != "":
    ...
    if totalminutes > 0:
        timereport = "Script execution time: " + str(totalminutes) + " minutes and " + str(
